...
class ExifTool():
    ...
    pattern_iptc = re.compile('^\[IPTC\]\s*([\w\s-]*):\s(.*)')
    pattern_xmp = re.compile('^\[XMP\]\s*([\w\s-]*):\s(.*)')
    pattern_pdf = re.compile('^\[PDF\]\s*([\w\s-]*):\s(.*)')
    IPTC = 0
    XMP = 1
    PDF = 2
    ...
    def __load(self, filename):
        ...
        self.xmp = {}
        self.pdf = {}
        p = subprocess.Popen( (self.config["exif"]["application"], '-G', '-charset', self.config["exif"]["charset"], filename), shell=False, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
        for line in p.stdout.readlines():
            m_iptc = self.pattern_iptc.match(line.rstrip('\n'))
            m_xmp = self.pattern_xmp.match(line.rstrip('\n'))
            m_pdf = self.pattern_pdf.match(line.rstrip('\n'))
            if m_iptc:
